...
def attention(query, key, value, mask=None):
    d_model = key.shape[-1]
    att_ = torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(d_model)
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, heads, d_model, dropout=0.1):
        ...
        self.q_l = nn.Linear(d_model, d_model, bias=False)
        self.k_l = nn.Linear(d_model, d_model, bias=False)
        self.v_l = nn.Linear(d_model, d_model, bias=False)
    ...
    def forward(self, q, k, v, mask=None):
        ...
        q = self.q_l(q).reshape(q.shape[0], -1, self.heads, self.d_k).transpose(1, 2)
        k = self.k_l(k).reshape(k.shape[0], -1, self.heads, self.d_k).transpose(1, 2)
        v = self.v_l(v).reshape(v.shape[0], -1, self.heads, self.d_k).transpose(1, 2)
...
class Encoder(nn.Module):
    ...
    def __init__(self, vocab_size, pad_idx, d_model, num_layers, heads, d_ff, dropout=0.1, max_seq_len=512):
        ...
        self.encode_layers = nn.ModuleList([EncoderLayer(heads, d_model, d_ff, dropout) for i in range(num_layers)])
...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, heads, d_model, d_ff, dropout=0.1, max_seq_len=512):
        super().__init__()
...
class Decoder(nn.Module):
    ...
    def __init__(self, vocab_size, pad_idx, d_model, num_layers, heads, d_ff, dropout=0.1, max_seq_len=512):
        ...
        self.embedding = nn.Embedding(vocab_size, d_model, pad_idx)
        self.position_encode = PositionEncoding(d_model, max_seq_len)
        self.decode_layers = nn.ModuleList([DecoderLayer(heads, d_model, d_ff, dropout) for i in range(num_layers)])
...
class Transformer(nn.Module):
    ...
    def generate_mask(self, query, key, is_triu_mask=None):
        device = query.device
